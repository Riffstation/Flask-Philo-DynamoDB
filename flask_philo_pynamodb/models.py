...
class FlaskPynamoDBModel(Model):
    ...
    def _get_connection(cls):
        ...
        if not hasattr(cls.Meta, 'request_timeout_seconds'):
            setattr(cls.Meta, 'request_timeout_seconds', env.get('request_timeout_seconds'))
        ...
        if not hasattr(cls.Meta, 'max_retry_attempts'):
            setattr(cls.Meta, 'max_retry_attempts', env.get('max_retry_attempts'))

        if not hasattr(cls.Meta, 'aws_access_key_id'):
            setattr(cls.Meta, 'aws_access_key_id', env.get('AWS_ACCESS_KEY_ID'))
        elif not cls.Meta.aws_access_key_id:
            setattr(cls.Meta, 'aws_access_key_id', env.get('AWS_ACCESS_KEY_ID'))

        if not hasattr(cls.Meta, 'aws_secret_access_key'):
            setattr(cls.Meta, 'aws_secret_access_key', env.get('AWS_SECRET_ACCESS_KEY'))
